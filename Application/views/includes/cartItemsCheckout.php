...
$collectionAddresses = $cartModel->getCollectionAddresses($userId);
$defaultCollectionAddress = $cartModel->getDefaultCollectionAddress($userId) ?: ($collectionAddresses[0] ?? null);

// Use shared shipping calculation function
// Keep backward compatibility wrapper for any code that might call getCourierGuyDeliveryCost
function getCourierGuyDeliveryCost($deliveryAddress, $cartItems)
{
    global $cartModel, $defaultCollectionAddress;
    $calculated = calculateCourierGuyShipping($deliveryAddress, $cartItems, $cartModel, $defaultCollectionAddress);
    // Fallback to 0 if calculation fails (web will handle display)
    return $calculated > 0 ? $calculated : 0;
}

$deliveryPrice = calculateCourierGuyShipping($address, $cartItems, $cartModel, $defaultCollectionAddress);
// Fallback to 0 if calculation fails (will be handled in display)
if ($deliveryPrice <= 0) {
    $deliveryPrice = 0;
}
?>
