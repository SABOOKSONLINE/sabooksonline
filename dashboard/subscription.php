          ...
          <div class="col-xl-8 offset-xl-2">
            <div class="shop_order_box mt60">
              ...
              <?php if($plan === 'Free'){
                    ...
                    include 'includes/select_token.php';
                   include 'includes/subscription_status.php'; 
              ...
              if ($plan == 'Free') {
                ...
                $payfast = '<a class="ud-btn btn-thm mt-2" href="includes/payfast/cancel?amount='.$price.'&type='.$type.'&token='.$token.'">Accept & Subscribe to Free Plan</a>';
				  
                echo '<a class="ud-btn btn-thm mt-2" href="../includes/backend/scripts/subscription-activate-free">Accept & Subscribe to Free Plan</a>';
				  
				  
          


              } else if ($payfast_status === 'ACTIVE'){
               // echo 'There is an active subscription';

                echo '<a class="ud-btn btn-thm mt-2" href="includes/payfast/update?amount='.$price.'&type='.$type.'&token='.$token.'">Update Your Subscription</a>';
            ...
            <!-- HTML Form for Coupon Redemption -->
            <form action="../includes/subscriptions/redeem_coupon.php" method="post" class="form-search row">     
                <label for="coupon_code">Enter Coupon Code:</label>
                <input type="text" id="coupon_code" name="coupon_code" class="form-control" required>

                <input type="hidden" name="user_email" value="<?php echo $_SESSION['ADMIN_EMAIL'];?>" required>

                <input type="hidden" name="userid" value="<?php echo $_SESSION['ADMIN_USERKEY'];?>" required>  

                <input type="hidden" name="item" value="<?php echo $_SESSION['ADMIN_USERKEY'];?>" required>

                <input type="hidden" name="id" value="<?php echo $_SESSION['ADMIN_USERKEY'];?>" required>

                <input type="hidden" id="user_email" name="amount" value="0" required>

                <input type="submit" class="btn-dark ud-btn btn-thm mt-2" value="Redeem Coupon">
            </form>
