          ...
          <?php include 'includes/mobile-guide.php';?>
            ...
            <hr>
							...
							if($_GET['result'] == "success"){
								echo "<div class='alert alert-success border-none'>Your Event has been successfully Uploaded!.</div>";
								} else if($_GET['result'] == "failed") {
									echo "<p class='text-warning'>Your <b>Event</b> could not be <b>Deleted</b>!</p>";
								}
								}

								if(isset($_GET['result1'])){
									if($_GET['result1'] == "success"){
										echo "<div class='alert alert-success border-none'>Your Event has been successfully UpdateD!</div>";
										} else if($_GET['result1'] == "failed") {
											echo "<p class='text-warning'>Your <b>Event</b> could not be <b>Updated</b>!</p>";
										}
										}

										if(isset($_GET['status'])){
											if($_GET['status'] == "success"){
												echo "<div class='alert alert-success border-none'>Your Event has been successfully Removed!</div>";
												} else if($_GET['status'] == "failed") {
													echo "<p class='text-warning'>Your <b>Event</b> could not be <b>Removed</b>!</p>";
												}
												}
            ...
            <div class="col-lg-9">
              <div class="dashboard_title_area">
                <h2>Manage Events</h2>
                <p class="text">You can manage, add or delete your event listings.</p>
              </div>
            </div>

            <div class="col-lg-3">
              <div class="text-lg-end">
                <a type="submit" class="ud-btn btn-dark default-box-shadow2" href="add-event" id="addevent">Create New Event<i class="fal fa-arrow-right-long"></i></a>
									...
									while($row = mysqli_fetch_assoc($result)) {
                                        ...
                                        $status = $row['STATUS'];
                                            ...
                                            if($duration <= 0){
                                                ...
                                                $duration_status = '- '.$duration.' Days Left';
                                            }

											if($status == "Active" || $status == "active"){
												$status = '<td class="vam"><span class="pending-style style1 bg-success text-white">'.ucwords($row['STATUS']).' '.$duration_status.'</span></td>';
											} else if($status == "Service Locked"){
												$status = '<td class="vam"><small class="pending-style style1 bg-danger text-white">'.ucwords($row['STATUS']).'</small></td>';
												...
												$status = '<td class="vam"><span class="pending-style style1 bg-secondary text-white">'.ucwords($row['STATUS']).' '.$duration_status.'</span></td>';
											}

                                            if($service_status == "Service Locked"){
												$service_status = '';
											}elseif($service_status == "Complete"){
												$service_status = '';
											} else {
                                                $service_status = '<a href="edit-event?contentid='.$row['CONTENTID'].'" class="icon me-2 text-success" data-bs-toggle="tooltip" data-bs-placement="top" title="Edit"><span class="flaticon-pencil"></span></a>';
                                            }
                                              ...
                                              <div class="freelancer-style1 p-0 mb-0 box-shadow-none">
                                                <div class="d-lg-flex align-items-lg-center">
                                                  <div class="thumb 0 position-relative rounded-circle mb15-md">
                                                    <img class="mx-auto" src="https://sabooksonline.co.za/cms-data/event-covers/'.$row['COVER'].'" data-src="cms-data/event-covers/'.$row['COVER'].'" alt="" width="50px">
                                                    <span class="online-badge2"></span>
                                                  </div>
                                                  
                                                  <div class="details ml15 ml0-md mb15-md">
                                                        <h5 class="title mb-2">'.ucwords($row['TITLE']).'</h5>
                                                        <p class="mb-0 fz14 list-inline-item mb5-sm pe-1"><i class="flaticon-place fz16 vam text-thm2 me-1"></i> '.ucwords($row['VENUE']).'</p>
                                                        <p class="mb-0 fz14 list-inline-item mb5-sm pe-1"><i class="flaticon-30-days fz16 vam text-thm2 me-1 bdrl1 pl15 pl0-xs bdrn-xs"></i> '.ucwords($row['EVENTDATE']).'</p>
                                                        <p class="mb-0 fz14 list-inline-item mb5-sm"><i class="flaticon-contract fz16 vam text-thm2 me-1 bdrl1 pl15 pl0-xs bdrn-xs"></i> 0 Received</p>
                                                    </div>
                                                </div>
                                              </div>
                                            </th>
                                            ...
                                            <!-- Modal -->
														...
														<div class="modal-dialog modal-dialog-centered border-none" style="border: none !important;">
															...
															<div class="modal-header">
																<h5 class="modal-title" id="exampleModalLabel">Are You Sure You want to delete the Event?</h5>
...
<script>
    ...
    $.post("../includes/backend/delete-event.php",
    {
        contentid: contentid,
        locate: locate
    },
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    tour.addStep({
        id: 'step1',
        text: '<b>Events Listings:</b><hr> You can upload, edit and manage your event listings.',
      ...
      tour.addStep({
        ...
        attachTo: { element: '#addevent', on: 'bottom' },
        classes: 'example-step-extra-class custom-step-content', // Add custom step class
        buttons: [
            {
                text: 'Skip',
