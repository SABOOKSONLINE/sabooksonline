<?php
            ...
            $sql = "SELECT * FROM book_stores WHERE ID = '$user'";
            $result = mysqli_query($conn, $sql);
            if(mysqli_num_rows($result) == false){
            }else{
                while($row = mysqli_fetch_assoc($result)) {

					$name = ucfirst($row['STORE_NAME']);
					$id = $row['ID'];
					$address = $row['STORE_ADDRESS'];
					$type = str_replace('-', ' ', ucfirst($row['STORE_AMNEMITIES']));
					$bio = $row['STORE_DESC'];
					$email = $row['STORE_EMAIL'];
					$number = $row['STORE_NUMBER'];
					$cover = str_replace('../../../../', '', $row['STORE_LOGO']);
					$images_gallery = $row['STORE_IMAGES'];
					$userkey = $row['STORE_USERID'];

                    $type =  '<li><a href="#0">'.str_replace(',','</a></li><li><a href="#">', $row['STORE_AMNEMITIES']).'</a></li>';
                    }
                }
...
<head>
    ...
    <link rel="icon" href="<?php 
		...
		} else { $cover = 'https://sabooksonline.co.za/'.$cover; }
	        ...
	        <div class="row">
	            ...
	            <div class="col-lg-4">
					...
					<?php 
                        ...
                        if (isset($_GET['store'])) {
                            $bookstore_id = $_GET['store']; // Replace this with the actual bookstore ID

                            $sql_fetch_images = "SELECT IMAGE FROM bookstores_images WHERE STOREID = ?";
                            ...
                            if ($result_images->num_rows > 0) {
                                while ($row = $result_images->fetch_assoc()) {
                                    ...
                                    echo '<figure><a href="https://sabooksonline.co.za/'.$image_path.'" title="Photo title" data-effect="mfp-zoom-in"><img src="https://sabooksonline.co.za/'.$image_path.'" alt="Image" class="lazy" alt=""/></figure></a>';
                                }
                            } else {
