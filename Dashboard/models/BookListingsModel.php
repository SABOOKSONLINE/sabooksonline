...
class BookListingsModel
{
    ...
    public function insertBook($data)
    {
        ...
        // Retrieve the CONTENTID that was generated by UUID()
        $sqlGetContentId = "SELECT CONTENTID FROM posts WHERE ID = ?";
        $stmtGetContentId = mysqli_prepare($this->conn, $sqlGetContentId);

        if (!$stmtGetContentId) {
            throw new Exception("Failed to prepare CONTENTID retrieval: " . mysqli_error($this->conn));
        }

        mysqli_stmt_bind_param($stmtGetContentId, "i", $insertedId);
        mysqli_stmt_execute($stmtGetContentId);
        mysqli_stmt_bind_result($stmtGetContentId, $contentId);
        mysqli_stmt_fetch($stmtGetContentId);
        mysqli_stmt_close($stmtGetContentId);

        // Store in session
        if (session_status() === PHP_SESSION_NONE) {
            session_start();
        }
        $_SESSION['last_inserted_content_id'] = $contentId;

        // Return both ID and CONTENTID
        return [
            'id' => $insertedId,
    ...
    {
        error_log("Attempting to delete book with CONTENTID: " . $contentId);

        mysqli_begin_transaction($this->conn);

        try {
            // Get book ID
            $sqlGetId = "SELECT ID FROM posts WHERE CONTENTID = ?";
            $stmtGetId = mysqli_prepare($this->conn, $sqlGetId);

            if (!$stmtGetId) {
                throw new Exception("Failed to prepare ID lookup: " . mysqli_error($this->conn));
            }

            mysqli_stmt_bind_param($stmtGetId, "s", $contentId);
            mysqli_stmt_execute($stmtGetId);
            mysqli_stmt_bind_result($stmtGetId, $bookId);
            mysqli_stmt_fetch($stmtGetId);
            mysqli_stmt_close($stmtGetId);

            if (!$bookId) {
                throw new Exception("No book found with CONTENTID: " . $contentId);
            }

            error_log("Found book ID: " . $bookId);

            // Get audiobook IDs
            $sqlGetAudiobookIds = "SELECT id FROM audiobooks WHERE book_id = ?";
            $stmtGetAudiobookIds = mysqli_prepare($this->conn, $sqlGetAudiobookIds);

            if ($stmtGetAudiobookIds) {
                mysqli_stmt_bind_param($stmtGetAudiobookIds, "i", $bookId);
                mysqli_stmt_execute($stmtGetAudiobookIds);
                $result = mysqli_stmt_get_result($stmtGetAudiobookIds);

                $audiobookIds = [];
                while ($row = mysqli_fetch_assoc($result)) {
                    $audiobookIds[] = $row['id'];
                }
                mysqli_stmt_close($stmtGetAudiobookIds);

                // Delete audiobook_chapters
                if (!empty($audiobookIds)) {
                    $placeholders = implode(',', array_fill(0, count($audiobookIds), '?'));
                    $sqlDeleteChapters = "DELETE FROM audiobook_chapters WHERE audiobook_id IN ($placeholders)";
                    $stmtDeleteChapters = mysqli_prepare($this->conn, $sqlDeleteChapters);

                    if ($stmtDeleteChapters) {
                        $types = str_repeat('i', count($audiobookIds));
                        mysqli_stmt_bind_param($stmtDeleteChapters, $types, ...$audiobookIds);
                        mysqli_stmt_execute($stmtDeleteChapters);
                        $deletedChapters = mysqli_stmt_affected_rows($stmtDeleteChapters);
                        error_log("Deleted {$deletedChapters} audiobook chapters");
                        mysqli_stmt_close($stmtDeleteChapters);
                    }
                }
            }

            // Delete audiobook_samples
            $sqlDeleteSamples = "DELETE FROM audiobook_samples WHERE book_id = ?";
            $stmtDeleteSamples = mysqli_prepare($this->conn, $sqlDeleteSamples);

            if ($stmtDeleteSamples) {
                mysqli_stmt_bind_param($stmtDeleteSamples, "i", $bookId);
                mysqli_stmt_execute($stmtDeleteSamples);
                $deletedSamples = mysqli_stmt_affected_rows($stmtDeleteSamples);
                error_log("Deleted {$deletedSamples} audiobook samples");
                mysqli_stmt_close($stmtDeleteSamples);
            }

            // Delete audiobooks
            $sqlDeleteAudiobooks = "DELETE FROM audiobooks WHERE book_id = ?";
            $stmtDeleteAudiobooks = mysqli_prepare($this->conn, $sqlDeleteAudiobooks);

            if ($stmtDeleteAudiobooks) {
                mysqli_stmt_bind_param($stmtDeleteAudiobooks, "i", $bookId);
                mysqli_stmt_execute($stmtDeleteAudiobooks);
                $deletedAudiobooks = mysqli_stmt_affected_rows($stmtDeleteAudiobooks);
                error_log("Deleted {$deletedAudiobooks} audiobooks");
                mysqli_stmt_close($stmtDeleteAudiobooks);
            }

            // Delete book_hardcopy
            $sqlDeleteHardcopy = "DELETE FROM book_hardcopy WHERE book_id = ?";
            $stmtDeleteHardcopy = mysqli_prepare($this->conn, $sqlDeleteHardcopy);

            if ($stmtDeleteHardcopy) {
                mysqli_stmt_bind_param($stmtDeleteHardcopy, "i", $bookId);
                mysqli_stmt_execute($stmtDeleteHardcopy);
                $deletedHardcopy = mysqli_stmt_affected_rows($stmtDeleteHardcopy);
                error_log("Deleted {$deletedHardcopy} hardcopy records");
                mysqli_stmt_close($stmtDeleteHardcopy);
            }

            // DELETE FROM CART - NEW!
            $sqlDeleteCart = "DELETE FROM book_cart WHERE book_id = ?";
            $stmtDeleteCart = mysqli_prepare($this->conn, $sqlDeleteCart);

            if ($stmtDeleteCart) {
                mysqli_stmt_bind_param($stmtDeleteCart, "i", $bookId);
                mysqli_stmt_execute($stmtDeleteCart);
                $deletedCart = mysqli_stmt_affected_rows($stmtDeleteCart);
                error_log("Deleted {$deletedCart} cart items");
                mysqli_stmt_close($stmtDeleteCart);
            }

            // Finally delete the main book
            ...
            if (!mysqli_stmt_execute($stmt)) {
                throw new Exception("Failed to delete book: " . mysqli_stmt_error($stmt));
            }

            $affectedRows = mysqli_stmt_affected_rows($stmt);
            mysqli_stmt_close($stmt);

            error_log("Deleted main book record. Affected rows: " . $affectedRows);

            if ($affectedRows === 0) {
                throw new Exception("Book deletion failed - no rows affected");
            }

            mysqli_commit($this->conn);
            error_log("Book and all related records deleted successfully");
