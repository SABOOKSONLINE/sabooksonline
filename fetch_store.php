...
if (isset($_GET['province']) && is_array($_GET['province']) && !empty($_GET['province'])) {
    ...
    $provinceWhereClause = "u.STORE_PROVINCE IN ('" . implode("', '", $escapedProvinces) . "')";
} else {
...
$query = "SELECT u.ID, u.STORE_NAME, u.STORE_ADDRESS, u.STORE_LOGO, u.STORE_AMNEMITIES
        FROM book_stores u
        WHERE $whereClause
        ORDER BY RAND()";
...
if ($result) {
    ...
    foreach ($userDetails as $username => $details) {
        ...
        <a href="store?store='.str_replace(' ', '_', strtolower($details['id'])).'" class="request text-center" type="button" id="'.str_replace(' ', '_', strtolower($username)).'_event" style="background-color: #e54750;color: #fff;">
