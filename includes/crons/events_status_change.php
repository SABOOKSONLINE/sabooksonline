...
try {
    // Update events with negative DURATION to 'Complete' status
    $sql = "UPDATE events SET STATUS = 'Complete' WHERE DURATION < 0 AND STATUS = 'Active'";
    ...
    if ($conn->query($sql)) {
        ...
        $logMessage = "Events cron ran successfully.";
