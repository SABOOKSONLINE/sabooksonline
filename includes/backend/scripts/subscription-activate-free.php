...
if ($conn->connect_error) {
    ...
    $_SESSION['payment'] = 'false';
        ...
        $subscriptionType = $_SESSION['subscription'];  
        ...
        if ($subscription_status) { // Assuming $subscription_status is set properly
            ...
            $stmt->bind_param("ss", $newPlan, $userkey);
            ...
            if ($stmt->execute()) {
                ...
                if ($resultCheckSubscription->num_rows > 0) {
                    ...
                    $next_invoice_date = 0; // Update with the new next invoice date
                    $amount = $sub_price; // Update with the new amount
                    //$plan = 'Free'; // Update with the new plan
                    //$status = 'CANCELLED'; // Update with the new plan     

                    $sqlUpdateSubscription = "UPDATE subscriptions_p SET amount = ?, status = ? WHERE user_id = ? AND status = 'active'";
                    $stmtUpdateSubscription = $conn->prepare($sqlUpdateSubscription);
                    $stmtUpdateSubscription->bind_param("sss", $amount, $newStatus, $userkey);   
