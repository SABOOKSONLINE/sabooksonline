...
<?php
    ...
    if (isset($subscriptionPrices[$plan])) {
        ...
        if ($stmt->execute()) {
            ...
            $logMessage = "Generated invoice for user $invoiceUser - Plan: $plan, Duration: $duration months, Total Price: $totalPrice";
            ...
            if ($logStmt->execute()) {
                ...
                if($plan == 'Free'){
                    echo "<script>Swal.fire({position: 'center',icon: 'success',title: 'Your Subscription has been Updated to ".$subscriptionType."!',showConfirmButton: false,timer: 6000});setInterval(function(){window.location.replace('page-dashboard-service-plan');},3000);</script>";
                } else {
                    header("Location: ../../../sabo/page-dashboard-service-plan");
