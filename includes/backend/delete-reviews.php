...
if (isset($_POST['contentid'])) {
    ...
    $sql = "DELETE FROM reviews WHERE ID = '$contentid'";

    if (mysqli_query($conn, $sql)) {
        //unlink('../../cms-data/event-covers/' . $contentlocate);
        echo "<script>Swal.fire({position: 'center',icon: 'success',title: 'Your Review has been Deleted!',showConfirmButton: false,timer: 6000});setInterval(function(){window.location.replace('reviews');},3000);</script>";
    } else {
